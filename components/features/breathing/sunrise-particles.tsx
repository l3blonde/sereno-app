...
function SunriseParticles({ phase, progress, isPaused }: SunriseSceneProps) {
    ...
    const particles = useMemo(() => {
        ...
        // Color palette for sunrise particles
        const sunColors = [
        ...
        for (let i = 0; i < count; i++) {
            // Position particles in a hemisphere
            ...
            // Assign colors - vary based on distance from center
            const distanceFromCenter = Math.sqrt(
                positions[i * 3] * positions[i * 3] + positions[i * 3 + 1] * positions[i * 3 + 1],
            )

            // Center particles are more yellow/gold, outer particles more orange/red
            const colorIndex = Math.min(Math.floor((distanceFromCenter / 7) * sunColors.length), sunColors.length - 1)

            const color = sunColors[colorIndex].clone()
    ...
    useFrame((_, delta) => {
        ...
        if (phase === "inhale") {
            ...
            sizeFactor = 1.0 + progress * 0.3
        } else if (phase === "exhale") {
            // Contract during exhale
            expansionFactor = -progress * 0.2
            sizeFactor = 1.3 - progress * 0.3
        } else if (phase === "hold1") {
            // Maximum expansion during hold after inhale
            expansionFactor = 0
            sizeFactor = 1.3
        } else {
            // Minimum expansion during hold after exhale
            expansionFactor = 0
            sizeFactor = 1.0
        }
        ...
        for (let i = 0; i < positions.count; i++) {
            ...
            const breath = {
                x: direction.x * expansionFactor,
                y: direction.y * expansionFactor,
                z: direction.z * expansionFactor,
            }

            // Update position
            positions.setXYZ(i, x + noise.x + breath.x, y + noise.y + breath.y, z + noise.z + breath.z)

            // Update size based on breathing
            const originalSize = particles.sizes[i]
            sizes.setX(i, originalSize * sizeFactor)
        }
...
function SunriseScene({
    ...
    return (
        <>
            ...
            <SunriseParticles
                phase={phase as "inhale" | "hold1" | "exhale" | "hold2"}
                progress={progress}
                isPaused={isPaused}
            />
        </>
...
export default function SimpleSunriseParticles({
                                                   phase = "inhale",
                                                   progress = 0,
                                                   isPaused = false,
                                                   isLightTheme = false,
